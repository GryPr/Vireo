...
class Portal(commands.Cog, name="portal"):
    ...
    async def initialize(self, inter: disnake.ApplicationCommandInteraction):
        ...
        embed = generic_embed(title="New portal created", description=f"Portal ID: {portal_id}",
    ...
    async def sub(self, inter: disnake.ApplicationCommandInteraction, portal_id: int):
        ...
        try:
            ...
            return
        embed = generic_embed(title="Added channel to the portal", description=f"Portal ID: {portal_id}",
                              author=inter.author.name)
        embed.add_field(
            name="Subscribe other servers using this command",
            value=f"/portal sub {portal_id}",
    ...
    async def unsub(self, inter: disnake.ApplicationCommandInteraction):
        try:
            ...
            await inter.send(embed=error_embed("Error has occurred adding the channel to the portal"))
            return
        embed = generic_embed(title="Removed channel from portal", description=f"Portal ID: {portal_id}",
                              author=inter.author.name)
        embed.add_field(
    ...
    async def id(self, inter: disnake.ApplicationCommandInteraction):
        ...
        if not portal_id:
            ...
            embed = generic_embed(title="Portal ID information", description=f"Portal ID: {portal_id}",
                                  author=inter.author.name)
            embed.add_field(
                name="Subscribe other servers using this command",
                value=f"/portal sub {portal_id}",
