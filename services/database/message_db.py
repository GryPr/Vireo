...
async def add_message(original_message_id: int, copy_message_id: int,
                      channel_id: int):
    driver_service.session.add(
        Message(original_message_id=str(original_message_id),
...
async def retrieve_copy_message(original_message_id: int, channel_id: int):
    result = driver_service.session.query(Message).filter_by(
...
async def retrieve_original_message(copy_message_id: int) -> int:
    result = driver_service.session.query(Message).filter_by(
        copy_message_id=str(copy_message_id)).one()
    return int(result.original_message_id)


# Retrieve message specific to channel
@retry(stop=stop_after_attempt(5), wait=wait_fixed(1))
async def retrieve_message_to_reply(original_message_id: int,
                                    channel_id: int) -> int:
    message_to_reply: Message = await retrieve_copy_message(
        original_message_id, channel_id)
...
async def retrieve_copy_messages(original_message_id: int) -> list[Message]:
    try:
        result = driver_service.session.query(Message).filter_by(
            original_message_id=str(original_message_id)).all()
